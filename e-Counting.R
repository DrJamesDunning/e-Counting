...
setwd("C:/Users/james/Documents/PhD/Teunis PhD followup/Code/Echo Counting Simulation/For Github")
source("bresenham.R")
...
SimFish <- T
Multiplier <- 10000 #multiplies the fish density inside DensData, disregard if SimFish = F
DensData <- read.csv("ExampleFishDensProfiles.csv") #only needed if SimFish <- T

Depth <- 10          #depth in m, overwritten if SimFish <- T
Distance <- 50      #length of transect in m
Speed_kts <- 6       #speed of boat in knots
PingInt <-  0.5      #ping interval in seconds
BeamAngle_Deg <- 7  #beam angle in degrees
MinTrack <- 3

FishD_m <- 0.001         #fish density in fish/m^3, overwritten if SimFish <- T
FishLayer <- c(6,1000) #No layer, fish are randomly distributed across full water column
#FishLayer <- c(3,7)
