...
#---- user input
Var_Depth <-     c(5,10,20,40,80)       #depth in m
Var_Depth <-     c(10,15,20)
Var_Distance <-  seq(29,29, by = 1)       #length of transect in m
...
Var_FishD_m <-   c(0.005, 0.01, 0.1,1)    #fish density in fish/m^3
Var_FishD_m <-   c(0.1, 1)

AddLayer  <- FALSE     #TRUE= use layer defined below, FALSE= disregard layer, have fish in all water column
FishLayer <- c(5,20)  #upper and lower limits of fish layer in m (the rest is empty water)

Var_Voxel <-     seq(0.1,0.1, by = 0.01)  #length of edge of voxel in m

Iterations <- 10         #how many times to repeat simulation for a given setting
...
ReportFreq <- 0.005            #how often to write a report (hrs)
...
for (it in 1:Iterations) {
  ...
  if (WriteReports & Sys.time() >= (start_time+NextReport*3600)) {
    writeLines(paste(ProgText,'\r',round(Perc),"% reached at ",Sys.time(),sep=""),".ProgressReport.txt")
...
Res$ArD_SED_trkXOff[v1,v2,3:4] <- se(itTable$ArD_SED_trkXOff)
  ...
  if (WriteReports & v1 == length(MovingVar1)) {
    ...
    writeLines(paste(v2,"full cycles out of",length(MovingVar2),"completed in",round(CycleTime,1),units(CycleTime),'\r',
              ...
              "ETA:", start_time+(CycleTime/v2)*length(MovingVar2)),".ETA_Processing.txt")
    writeLines(paste(ProgText,'\r',round(Perc),"% reached at ",Sys.time(),sep=""),".ProgressReport.txt")
  }
}
...
# 
filename <- '4Depths_4FishD_50_FTracks.RData'
setwd ("R:/CLSM/School of Biological Sciences/FisheriesGroup/PhD students/James Dunning/James Code/Echo Counting Simulation")
#save.image(file=filename)
...
if (WriteReports){
writeLines(paste("End of processing reached at ",Sys.time(),'\r',"Cheerio!", sep=""),".End_of_processing.txt")
}
}
end_time - start_time
